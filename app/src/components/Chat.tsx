...
export default function Chat() {
    const {supabaseClient: client} = useSupabase()
    ...
    const sendMessageAndReceiveResponse = useMutation({
        mutationFn: async (userMessage: Message) => {
            ...
            const {error: sendMessageError} = await supabaseClient
                .from('conversations')
    ...
    const newConversation = useMutation({
        mutationFn: async () => {
            ...
            const {data, error} = await supabaseClient
                ...
                .insert([
                    {
                        context: [{role: 'assistant', content: 'Hey, how can I help you?'}]
                    }
                ])
                .select('*')
            if (error) {
        ...
        queryFn: async () => {
            if (conversationId === null) {
                const {
                    data,
                    error
                } = await supabaseClient.from('conversations').select('*').order('created_at', {ascending: false}).limit(1)
                if (error) {
                    throw error
                }
                ...
                const {
                    data,
                    error
                } = await supabaseClient.from('conversations').select('*').eq('id', conversationId).single()
                if (error) {
                    throw error
                }
                return data
    ...
    useEffect(() => {
        if (supabaseClient && getConversation.data) {
            const nextMessages = getConversation.data.context
            setMessages(nextMessages)
        }
    }, [supabaseClient, getConversation.data])
    ...
    return (
        <>
            <div
                className='from-background fixed top-0 flex h-24 w-full items-center justify-between bg-gradient-to-b'></div>
            <div className='fixed left-4 top-4 flex space-x-4'>
                <SideMenu supabaseClient={supabaseClient} setConversationId={setConversationId}/>
            </div>
